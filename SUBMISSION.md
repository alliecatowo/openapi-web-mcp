...
| 3b | Open source license file, detectable at top of repo page | ❌ **FAILING** | Repo is public, but `LICENSE` holds only the Apache-2.0 *header stub* (710 bytes), not the full licence text. GitHub's detector reports `spdx_id: NOASSERTION`, `license.name: "Other"` — the sidebar does **not** read "Apache-2.0". Verified 2026-09-02 via `api.github.com/repos/alliecatowo/openapi-web-mcp`. Fix: replace `LICENSE` with the full Apache-2.0 text. See CHECKLIST. |
